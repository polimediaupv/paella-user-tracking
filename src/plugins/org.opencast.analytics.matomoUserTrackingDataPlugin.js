 ...
 */
 ...
 export default class OpencastMatomoTrackingDataPlugin extends DataPlugin {
   ...
   async load(){
     ...
     loadExternalScript(server + tracking_client + '.js').then((matomo) => {
   ...
   async write(context, { id }, data) {
     ...
     this.player.log.debug(JSON.stringify(this.player.videoManifest));
     this.player.log.debug(`Video title: ${ this.player.videoManifest.metadata.title }`);
     const opencastLog = {
       id,
